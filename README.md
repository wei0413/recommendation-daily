...
从2026年9月11日开始，每天北京时间上午10：00，自动从arxiv检索最新论文。（tips：有些论文不挂arxiv，所以针对特定会议，例如SIGIR/Recsys等，后续针对优化一下，多录入进来）。或者可以直接去检索这些会议官网见刊的最新论文。
...
GitHub Actions 会在工作日上午 10:00 运行，更新数据后部署到 GitHub Pages；也可以在 Actions 页面手动触发。没有配置大模型密钥时，论文抓取、分类和发布仍会运行，只跳过中文笔记生成。
